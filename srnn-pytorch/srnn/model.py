...
class HumanNodeRNN(nn.Module):
    ...
    def forward(self, pos, h_temporal, h_spatial_other, h, c):
        ...
        h_edges = torch.cat((h_temporal, h_spatial_other), 2)
        h_edges_embedded = self.relu(self.edge_attention_embed(h_edges))
        h_edges_embedded = self.dropout(h_edges_embedded)

        concat_encoded = torch.cat((encoded_input, h_edges_embedded), 2)

        # One-step of LSTM
        concat_encoded,h,c = concat_encoded.reshape(-1,concat_encoded.shape[-1]),h.reshape(-1,h.shape[-1]),c.reshape(-1,c.shape[-1])
        ...
        return out.reshape(-1,pos.shape[1],out.shape[-1]), h_new, c_new
...
class SRNN(nn.Module):
    ...
    def forward(self, nodes, edges, nodesPresent, edgesPresent, hidden_states_node_RNNs, hidden_states_edge_RNNs, cell_states_node_RNNs, cell_states_edge_RNNs):
        ...
        keypoints = nodes.shape[2]
        # Initialize output array
        outputs = Variable(torch.zeros(self.seq_length*numNodes, keypoints,self.output_size)).cuda()
        ...
        for framenum in range(self.seq_length):
            ...
            if len(edgeIDs) != 0:
                ...
                if len(temporal_edges) != 0:
                    ...
                    # print('List of temporal edges........',list_of_temporal_edges.shape)
                    ...
                    cell_states_edge_RNNs[list_of_temporal_edges.data] = c_temporal.reshape(-1,keypoints,c_temporal.shape[-1])
