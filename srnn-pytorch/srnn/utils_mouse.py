...
class DataLoaderMouse():
    ...
    def __init__(self, batch_size=50, seq_length=5,forcePreProcess=False, infer=False, body_keypoint:list = None,train_frac = 0.0625,val_fraction=0.3,\
                 skip_frames_by_static = 'Slice',normalise = True,frames = 150,train_data = False,oob_indices=None,seed = 903809718,allkp = False):
        '''
        ...
        if not train_data:self.sequences = [sequences[cur] for i,cur in enumerate(range(len(sequences))) if cur in oob_indices and i<=50]
        # print("Sequences Used:" ,self.sequences)
        ...
        self.skip_frames_by_static = skip_frames_by_static
        # Store the arguments
    ...
    def frame_preprocess(self,data_file,thresh=100,skip = 15):
        '''
        ...
        for index, seq_id in enumerate(tqdm(self.sequences,desc='Processing Sequence ......')):
            ...
            randomidx = np.random.randint(0,len(frameList)-self.frame)
            ...
            for ind, frame in enumerate(frameList):
                ...
                # if self.normalise:
                #     miceInFrame = self.convert_seq(miceInFrame)
               
                if self.skip_frames_by_static == 'Static':
                    if not (abs(miceInFrame-curr_frame)>thresh).any():
                        continue
                    curr_frame = miceInFrame

                elif self.skip_frames_by_static == 'Slice':
                    if ind < randomidx:continue
                    else:
                        if slice_ind%self.frame == 0 and slice_ind != 0:break
                        slice_ind+=1
                else:
                    if ind % skip != 0:
                        continue
