...
def main():
    ...
    parser.add_argument('--save_dir', type=int, default=0,
    ...
    # Save directory
    save_directory = 'save/'
    save_directory += str(sample_args.save_dir)+'/'
    ...
    # dataloader = DataLoaderMouse(1, sample_args.pred_length + sample_args.obs_length,True)
    dataloader = DataLoader(1, sample_args.pred_length + sample_args.obs_length,dataset,True,True)
    

    dataloader.reset_batch_pointer()

    # Construct the ST-graph object
    stgraph = ST_GRAPH(1, sample_args.pred_length + sample_args.obs_length,body_keypoint=[0])
...
def sample(nodes, edges, nodesPresent, edgesPresent, args, net, true_nodes, true_edges, true_nodesPresent):
    ...
    for tstep in range(args.obs_length-1, args.pred_length + args.obs_length-1):
        ...
        # if tstep == args.pred_length + args.obs_length-2:
        #     x_arr,y_arr = np.zeros((100,nodes.shape[1])),np.zeros((100,nodes.shape[1]))
        #     for i in range(100):
        #         next_x, next_y = sample_gaussian_2d(mux.data, muy.data, sx.data, sy.data, corr.data, nodesPresent[args.obs_length-1])
        #         x_arr[i,:],y_arr[i,:] = next_x,next_y


        # if tstep == args.pred_length + args.obs_length-2:
        #     x_arr,y_arr = np.zeros((1000,nodes.shape[1])),np.zeros((1000,nodes.shape[1]))
        #     for i in range(1000):
        #         next_x, next_y = sample_gaussian_2d(mux.data, muy.data, sx.data, sy.data, corr.data, nodesPresent[args.obs_length-1])
        #         x_arr[i,:],y_arr[i,:] = next_x,next_y
