...
def train(args):
    ...
    if args.wandb:
        ...
        wandb.login()
        # Wandb initialize
        run = wandb.init(project="Summer Research", ## Name of the project 
                ...
                name="New Version of Pedestrian Training" # Name of the session
                )
    # Construct the ST-graph object
    stgraph = ST_GRAPH(1, args.seq_length + 1,body_keypoint=[0])
    # Log directory
    log_directory = 'log/'
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(dataloader.num_batches):
            ...
            for sequence in range(dataloader.batch_size):
                ...
                # Reset the stgraph
                stgraph.reset()
                # df = pd.DataFrame(attn_weights[-1][0][0].T,columns=['Mice 2', 'Mice 3'])
            end = time.time()
        ...
        if args.wandb:wandb.log({"Validation Loss":loss_batch})
...
def attention_visualization(nodes,ret_nodes,attn_weights,nodesPresent,upto=0,peds = 5,color_dict = None,prediction=False,observed_length=8):
    ...
    if upto !=0:marker,linestyle = upto,'o','dashed' 
    else:marker,linestyle = upto,'o','dashed' 
    ...
    if upto != 0:    
        for frames in range(nodes.shape[0]+1):
            ...
            for frame_id in range(frames):
                current_frame = nodes__[frame_id,:,:]
        ...
        circle = plt.Circle((traj_ped_o[-1, 1], traj_ped_o[-1, 0]), 0.01, fill=False, color='r', linewidth=2)
